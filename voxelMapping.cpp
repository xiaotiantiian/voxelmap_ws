...
int pcd_save_interval = 0;                               // 保存间隔(帧数)
int pcd_index = 0;                                       // PCD文件索引
int scan_wait_num = 0;                                   // 等待计数器
PointCloudXYZI::Ptr pcl_wait_save(new PointCloudXYZI()); // 待保存的点云
...
double max_voxel_size = 1.0; // 最大体素大小
...
void livox_pcl_cbk(const livox_ros_driver2::CustomMsg::ConstPtr &msg)
{
  ...
  else if (sweep_mode == 2 && meas_num >= 10)
  {
...
void savePCDToFile()
{
  ...
  {
    pcd_index++;
    std::string file_path(string(string(ROOT_DIR) + "PCD/scans_") + to_string(pcd_index) + string(".pcd"));
    pcl::PCDWriter pcd_writer;
    pcd_writer.writeBinary(file_path, *pcl_wait_save);
    std::cout << "Saved PCD to: " << file_path << std::endl;
    pcl_wait_save->clear();
  }
}

// 将点云从 LiDAR 坐标系转换到世界坐标系并发布到 ROS 话题  保存点云
                         ...
                         const int point_skip)
{
  ...
  if (pcd_save_en)
  {
    ...
    laserCloudWorld->reserve(size / 3 + 1); // 预分配内存

    // 三采一降采样并转换到世界坐标系
    for (int i = 0; i < size; i += 3) // 步长为3
    {
    ...
    *pcl_wait_save += *laserCloudWorld; // 累积到待保存点云
    scan_wait_num++;
  }
  // save_pcd
  // if (pcd_save_en)
...
// 发布有效的点云特征（与体素地图中的平面匹配的点）以及可视化的点到平面连线
...
// 发布未被体素地图匹配的点云（无效点云），用于调试或分析未被使用的点
...
// 发布有效点云（与体素地图匹配的点），并为每个点分配颜色（基于强度值）
...
// 发布 SLAM 系统的里程计数据（位姿），并广播 TF 变换，用于描述 LiDAR 在世界坐标系中的位置和姿态
...
int main(int argc, char **argv)
{
  ...
  // pcd_save
  nh.param<bool>("pcd_save/enable", pcd_save_en, false);
  nh.param<int>("pcd_save/interval", pcd_save_interval, 0);
  ...
  nh.param<int>("sweepparam/sweep_mode", sweep_mode, 0); // 扫描模式
  ...
  if (read_mode == 1)
  {
    ...
    if (lidar_rostopic == 0)
      bag_reader->registerLivoxCallback([&](const livox_ros_driver2::CustomMsg::ConstPtr &msg)
                                        { livox_pcl_cbk(msg); });
    else
  ...
  // for Plane Map
  bool init_map = false;
  std::unordered_map<VOXEL_LOC, OctoTree *> voxel_map; // 哈希表，key为体素坐标，value为OctoTree节点
  ...
  while (status)
  {
    ...
    if (sync_packages(Measures))
    {
      ...
      // {
      //   if (i % 100 == 0 && scanIdx == 0)
      //     cout << "point" << i << " " << Measures.lidar->points[i].x << " " << Measures.lidar->points[i].y << " " << Measures.lidar->points[i].z << endl;
      // }
      // 点云预处理
      ...
      // {
      //   if (i % 100 == 0 && scanIdx == 0)
      //     cout << "point" << i << " " << feats_undistort->points[i].x << " " << feats_undistort->points[i].y << " " << feats_undistort->points[i].z << endl;
      // }
      ...
      if (flg_EKF_inited && meas_num > 3 && sweep_mode != 0 && conbined_mode == 1)
      {
        ...
        lidar_processing->R = p_imu->Lid_rot_to_IMU;
        lidar_processing->T = p_imu->Lid_offset_to_IMU;
        // 调用融合函数
      ...
      if (flg_EKF_inited && !init_map)
      {
        ...
        for (size_t i = 0; i < world_lidar->size(); i++)
        {
          ...
          // if (i % 100 == 0)
          // {
          //   cout << "point" << i << ": " << point_this.transpose() << endl;
          // }
          ...
          // 协方差用于点云匹配和占用概率更新，确保地图对噪声的鲁棒性
          calcBodyCov(point_this, ranging_cov, angle_cov, cov); // 计算点的协方差 考虑测距误差和视角误差
          ...
          pv.cov = cov;
          pv_list.push_back(pv); // 包含点坐标和协方差
      ...
      std::cout << " feats size:" << feats_undistort->size()
                << ", down size:" << feats_down_body->size()
                << std::endl;
      auto t_downsample =
          std::chrono::duration_cast<std::chrono::duration<double>>(
      ...
      for (iterCount = 0; iterCount < NUM_MAX_ITERATIONS; iterCount++)
      {
        ...
        // 将降采样后的点云从lidar坐标系转换到世界坐标系
        transformLidar(state, p_imu, feats_down_body, world_lidar);
        // 为每个点生成 pointWithCov，包含世界坐标（point_world）和协方差（cov）。
        for (size_t i = 0; i < feats_down_body->size(); i++)
        {
          pointWithCov pv;
        ...
        // 点到平面残差  匹配点云到体素地图中的平面，生成 ptpl_list（包含点到平面的信息（point, normal, center, d））
        BuildResidualListOMP(voxel_map, max_voxel_size, 3.0, max_layer, pv_list,
                             ptpl_list, non_match_list);

        for (size_t i = 0; i < ptpl_list.size() && i < 5; i++)
        {
          if (ptpl_list[i].normal.array().isNaN().any() || ptpl_list[i].center.array().isNaN().any() ||
              std::isnan(ptpl_list[i].d) || ptpl_list[i].plane_cov.array().isNaN().any())
          {
            ROS_ERROR("[scanIdx:%d, iter:%d] Invalid ptpl at index %zu: normal=(%f,%f,%f), center=(%f,%f,%f), d=%f",
                      scanIdx, iterCount, i,
                      ptpl_list[i].normal(0), ptpl_list[i].normal(1), ptpl_list[i].normal(2),
                      ptpl_list[i].center(0), ptpl_list[i].center(1), ptpl_list[i].center(2),
                      ptpl_list[i].d);
          }
        }
         ...
         * ***/
        // 计算残差和雅可比矩阵
        MatrixXd Hsub(effct_feat_num, 6);         // 雅可比矩阵 6维状态量（rot pos）
        MatrixXd Hsub_T_R_inv(6, effct_feat_num); // 加权雅可比转置
        VectorXd R_inv(effct_feat_num);           // 测量协方差的倒数
        VectorXd meas_vec(effct_feat_num);        // 测量残差
        ...
        for (int i = 0; i < effct_feat_num; i++)
        {
          ...
          {
            std::stringstream ss;
            ss << cov.format(Eigen::IOFormat(4));
            ROS_ERROR("[scanIdx:%d, iter:%d] Invalid cov at index %d: \n%s",
                      scanIdx, iterCount, i, ss.str().c_str());
          }
          Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> cov_solver(cov);
          if (cov_solver.eigenvalues().minCoeff() <= 0)
          {
            ROS_ERROR("[scanIdx:%d, iter:%d] Non-positive definite cov at index %d: eigenvalues=(%f,%f,%f)",
                      scanIdx, iterCount, i,
                      cov_solver.eigenvalues()(0), cov_solver.eigenvalues()(1), cov_solver.eigenvalues()(2));
          ...
          if (std::isnan(R_inv(i)) || std::isinf(R_inv(i)))
          {
            ROS_ERROR("[scanIdx:%d, iter:%d] Invalid R_inv at index %d: R_inv=%f, sigma_l=%f, norm_cov=%f",
                      scanIdx, iterCount, i, R_inv(i), sigma_l, norm_vec.transpose() * cov * norm_vec);
          }
        ...
        // 检查 Hsub 是否包含 nan
        if (Hsub.array().isNaN().any())
        {
          ROS_ERROR("[scanIdx:%d, iter:%d] Invalid Hsub detected", scanIdx, iterCount);
          for (int i = 0; i < Hsub.rows() && i < 5; i++)
          { // 打印前5行以避免日志过多
            ROS_ERROR("Hsub row %d: [%f, %f, %f, %f, %f, %f]", i,
                      Hsub(i, 0), Hsub(i, 1), Hsub(i, 2), Hsub(i, 3), Hsub(i, 4), Hsub(i, 5));
          }
        }

        // 检查 meas_vec 是否包含 nan
        if (meas_vec.array().isNaN().any())
        {
          ROS_ERROR("[scanIdx:%d, iter:%d] Invalid meas_vec detected", scanIdx, iterCount);
          for (int i = 0; i < meas_vec.size() && i < 5; i++)
          { // 打印前5个元素
            ROS_ERROR("meas_vec[%d]: %f", i, meas_vec(i));
          }
        }

        // 检查 Hsub_T_R_inv 是否包含 nan
        if (Hsub_T_R_inv.array().isNaN().any())
        {
          ROS_ERROR("[scanIdx:%d, iter:%d] Invalid Hsub_T_R_inv detected", scanIdx, iterCount);
          for (int i = 0; i < Hsub_T_R_inv.cols() && i < 5; i++)
          { // 打印前5列
            ROS_ERROR("Hsub_T_R_inv col %d: [%f, %f, %f, %f, %f, %f]", i,
                      Hsub_T_R_inv(0, i), Hsub_T_R_inv(1, i), Hsub_T_R_inv(2, i),
                      Hsub_T_R_inv(3, i), Hsub_T_R_inv(4, i), Hsub_T_R_inv(5, i));
          }
        }

        // 检查 state.cov 的正定性
        Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> cov_solver(state.cov);
        if (cov_solver.eigenvalues().minCoeff() <= 0)
        {
          ROS_ERROR("[scanIdx:%d, iter:%d] state.cov is not positive definite: min eigenvalue=%f",
                    scanIdx, iterCount, cov_solver.eigenvalues().minCoeff());
        }
        ...
        {
          ROS_ERROR("[scanIdx:%d, iter:%d] Invalid K detected", scanIdx, iterCount);
          for (int i = 0; i < K.rows() && i < 5; i++)
          { // 打印前5行
            ROS_ERROR("K row %d: [%f, %f, %f, %f, %f, %f]", i,
                      K(i, 0), K(i, 1), K(i, 2), K(i, 3), K(i, 4), K(i, 5));
          }
        }
      }

      // cout << "state: " << state.pos_end.transpose() << " "
      //      << state.pos_end.norm() << "\n"
      //      << endl;
      p_imu->save_imu_state(Measures.lidar_beg_time, state.pos_end, state.rot_end);
      ...
      pcl::PointCloud<pcl::PointXYZI>::Ptr world_lidar(
          new pcl::PointCloud<pcl::PointXYZI>);
      transformLidar(state, p_imu, feats_down_body, world_lidar);
      std::vector<pointWithCov> pv_list; // 点云点及其协方差，包含点的不确定性
      for (size_t i = 0; i < world_lidar->size(); i++)
      {
        pointWithCov pv;
        pv.point << world_lidar->points[i].x, world_lidar->points[i].y,
