...
public:
  ...
  void save_imu_state(double timestamp, const V3D &pos, const M3D &rot);
...
void ImuProcess::save_imu_state(double timestamp, const V3D &pos, const M3D &rot)
{
  // 构建目录路径
  std::stringstream dir_ss;
  // dir_ss << "/home/tian/workspace/fast_lio_bspline_new2/bs/"
  //        << state_queue_size << "/";
  dir_ss << string(ROOT_DIR) << "pos/";
                          ...
                          int &N)
{
  ...
  state_inout.rot_end = R_wi;                                        // 覆盖初始姿态的旋转部分
  state_inout.gravity = -R_wi * mean_acc / mean_acc.norm() * G_m_s2; // 更新grav 这个不更新的话 imu初始会有漂移
  /* 重力对齐 */
                              ...
                              PointCloudXYZI &pcl_out)
{
  ...
  // for (int i = 0; i < 3; i++)
  // {
  //   cout << "imu_time_" << i << ":" << std::fixed << std::setprecision(6) << v_imu[i]->header.stamp.toSec() << endl;
  // }

  // for (int i = v_imu.size() - 3; i < v_imu.size(); i++)
  // {
  //   cout << "imu_time_" << i << ":" << std::fixed << std::setprecision(6) << v_imu[i]->header.stamp.toSec() << endl;
  // }
  ...
  cout << "pcl_out size: " << pcl_out.points.size() << endl;
  sort(pcl_out.points.begin(), pcl_out.points.end(), time_list);
  const double &pcl_end_time =
      pcl_beg_time + pcl_out.points.back().curvature / double(1000);
  cout << fixed << setprecision(6) << "[ IMU Process ]: Process lidar from " << pcl_beg_time << " to " << pcl_end_time << ", "
  ...
  cout << "state_inout: " << std::fixed << std::setprecision(6)
       << pcl_end_time << " "
       << state_inout.pos_end.transpose() << " "
       << state_inout.pos_end.norm() << "\n";
