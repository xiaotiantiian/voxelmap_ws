    ...
    const livox_ros_driver2::CustomMsg::ConstPtr &msg)
{
  ...
  // std::cout << "Raw point cloud size: " << plsize << std::endl;
  ...
  //   {
  //     after_scanline_check++;
  //   }
  // }
  // std::cout << "After scanline check, valid points: "
            // << after_scanline_check << std::endl;

  // uint after_downsample = 0;
  // uint after_blind_check = 0;
  for (uint i = 1; i < plsize; i++)
  {
    if ((msg->points[i].line < N_SCANS) &&
        ((msg->points[i].tag & 0x30) == 0x10 ||
         (msg->points[i].tag & 0x30) == 0x00))
    {
      ...
      if (i % point_filter_num == 0)
      {
        ...
        //          blind * blind)) // ???z+z??? bug   ？？？if ( (A) || (B) || (C && D) )？？？ &&优先级高于||
        if ((abs(pl_full[i].x - pl_full[i - 1].x) > 1e-7 ||
             abs(pl_full[i].y - pl_full[i - 1].y) > 1e-7 ||
             abs(pl_full[i].z - pl_full[i - 1].z) > 1e-7) &&
            (pl_full[i].x * pl_full[i].x +
                 pl_full[i].y * pl_full[i].y +
                 pl_full[i].z * pl_full[i].z >
             blind * blind))//(A || B || C) && D
        {
          // if (i == 0 || i == 100 || i == 500 || i == 1000)
          //   std::cout << "Point " << i << ": "
          //             << "x: " << pl_full[i].x
          //             << ", y: " << pl_full[i].y
          //             << ", z: " << pl_full[i].z
          //             << ", intensity: " << pl_full[i].intensity
          //             << ", curvature: " << pl_full[i].curvature
          //             << std::endl;
          is_valid_pt[i] = true;
          // after_blind_check++;
        }
  ...
  // std::cout << "After downsample, valid points: " << after_downsample
  //           << std::endl;
  // std::cout << "After blind check, valid points: " << after_blind_check
  //           << std::endl;
